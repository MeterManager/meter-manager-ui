...
import {
  ...
  Box, Stack, IconButton, Tooltip, TextField,
...
const ActsTable = () => {
  ...
  return (
    <Box p={2}>
      ...
      {error && <Typography color="error"  mb={2} sx={{ bgcolor: '#ffebee', p: 2, borderRadius: 1 }}>{error}</Typography>}
      {loading ? (
        ...
        <Stack direction="row" spacing={4} flexWrap="wrap">
          ...
          <Typography>
            <BoltOutlinedIcon sx={{ verticalAlign: "middle", color: theme.palette.primary.main, mr: 0.5 }} />
              Загальне споживання:{" "}
              <strong>{categorySummary.totalConsumption.toFixed(2)}</strong>
          </Typography>
        </Stack>
