...
export const getDialogMessage = (action, dependencies, entity = 'location') => {
  ...
  if (meterDependencyKey && dependencies?.[meterDependencyKey]) {
     parts.push(`${dependencies[meterDependencyKey]} ${pluralize(dependencies[meterDependencyKey], UA.confirm_active_meters, UA.confirm_active_meters_plural, UA.confirm_active_meters_genitive)}`);
  }

  if (entity === 'location' && dependencies?.deliveries) {
    parts.push(`${dependencies.deliveries} ${pluralize(dependencies.deliveries, UA.confirm_deliveries, UA.confirm_deliveries_plural, UA.confirm_deliveries_genitive)}`);
  }
  if (entity === 'location' && dependencies?.active_tenants) {
    parts.push(`${dependencies.active_tenants} ${pluralize(dependencies.active_tenants, UA.confirm_active_tenants, UA.confirm_active_tenants_plural, UA.confirm_active_tenants_plural)}`);
  }
